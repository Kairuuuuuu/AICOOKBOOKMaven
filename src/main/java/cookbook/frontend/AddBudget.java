...
import cookbook.backend.BudgetService;
...
public class AddBudget {
    ...
    public static void showBudgetMenu(JFrame parentFrame) {
        ...
        popupPanel.setBounds(45, 312, 300, 225); 
        ...
        String currentText = CookbookState.currentBudget.equals("Php 0.00") ? "Enter Budget" : CookbookState.currentBudget.replace("Php ", "");
        ...
        // --- SEPARATED LOGIC DELEGATION ---
        saveBtn.addActionListener(e -> {
            String newBudgetStr = budgetField.getText().trim();
            
            // Pass the data to the backend to validate
            BudgetService.BudgetResult result = BudgetService.validateBudget(newBudgetStr, CookbookState.currentTotalCost);
            
            if (!result.isSuccess) {
                // If it failed, show the backend's error message
                errorLabel.setText(result.errorMessage);
                errorLabel.setVisible(true);
            } else {
                // If it succeeded, apply the formatted string to the UI
                CookbookState.currentBudget = result.formattedBudget;
                
                if (MainMenu.budgetLabel != null) {
                    MainMenu.budgetLabel.setText("Budget: " + CookbookState.currentBudget);
                }
